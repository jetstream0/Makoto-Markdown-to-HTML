...
exports.parse_md_to_html = exports.parse_md_to_html_with_warnings = void 0;
//some minor differences with markdown spec?
function parse_md_to_html_with_warnings(md) {
    let html = "";
    let html_line = "";
    let warnings = [];
    let line_number = 1;
    ...
    let header_num = 0;
    let asterisk_num = 0;
    let asterisk_out_num = 0;
    let in_asterisk = false;
    let horizontal_num = 0;
    let horizontal_rule = false;
    let was_image = false;
    let image_alt = undefined;
    let image_src = undefined;
    let was_link = false;
    let link_content = undefined;
    let link_href = undefined;
    let in_code = false;
    let in_code_block = false;
    let first_line_code_block = false;
    let code_block_lang = undefined;
    let space_start = false;
    let in_blockquote = false;
    let in_unordered_list = false;
    let blockquote_list = false;
    let in_ordered_list = false;
    let ordered_list_num = 0;
    ...
    for (let i = 0; i < chars.length; i++) {
        ...
        if (char === "\n" || i === chars.length - 1) {
            ...
            let ol_num_length = String(ordered_list_num + 1).length;
...
exports.parse_md_to_html_with_warnings = parse_md_to_html_with_warnings;
function parse_md_to_html(md) {
    return parse_md_to_html_with_warnings(md).html;
}
exports.parse_md_to_html = parse_md_to_html;
